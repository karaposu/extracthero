...
class FilterHero:
    ...
    def _dispatch(
        ...
        if len(it_list) == 1 or not separate:
            ...
            generation_result = self.llm.filter_via_llm(corpus_str, prompt, filter_strategy=filter_strategy)
    ...
    async def run_async(
        ...
        reduce_html: bool = True,
        enforce_llm_based_filter: bool = False,
        filter_strategy: str = "liberal",  # ← ADD HERE
    ) -> FilterOp:
        """
        ...
        gen_results = await self._dispatch_async(
            ...
            extraction_spec, 
            filter_separately,
            filter_strategy  # ← PASS IT HERE
        )
       

        # 5) success flag
        if isinstance(extraction_spec, WhatToRetain) or not filter_separately:
            # Single spec or combined processing → check first result only
            ok = gen_results[0].success
        else:
            # Multiple specs with separate processing → all must succeed
            ok = all(r.success for r in gen_results)
              
        
        # 6) aggregate
        content_final, usage_final = self._aggregate( gen_results, extraction_spec, filter_separately)

        
        filtered_data_token_size = None
        if ok and content_final is not None and isinstance(content_final, str):
            try:
                
                filtered_data_token_size = len(encoding.encode(content_final))
            except Exception as e:
    ...
    async def _dispatch_async(
        ...
        if len(item_list) == 1 or not separate:
            what_to_retain_merged_prompt = "\n\n".join(it.compile() for it in item_list)

            generation_result = await self.llm.filter_via_llm_async(
                corpus_str, 
                what_to_retain_merged_prompt,
                filter_strategy=filter_strategy  
            )
...
if __name__ == "__main__":
    ...
    # filter_op = filter_hero.run(html_doc, specs, text_type="html",filter_strategy="liberal")
    # filter_op = filter_hero.run(html_doc, specs, text_type="html", filter_strategy="inclusive")
    # filter_op = filter_hero.run(html_doc, specs, text_type="html", filter_strategy="contextual")
    filter_op = filter_hero.run(html_doc, specs, text_type="html", filter_strategy="recall")
    ...
    #     print("original token size:", filter_op.html_reduce_op.total_token)
    #     print("reduced token size:", filter_op.html_reduce_op.reduced_total_token)
    #     print("token_reducement_percentage:", filter_op.html_reduce_op.token_reducement_percentage)
    #     print("", )
    #     print("filtered_data_token_size:", filter_op.filtered_data_token_size)

        # print("", )
        # print("html_reduced_data:", filter_op.reduced_html)
        # path=filter_op.save_reduced_html_to_txt(filename="reduced_html",dir="./extracthero/real_life_samples/1")
        # print("path:", path)
